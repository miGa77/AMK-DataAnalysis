...
&rightarrow; Lösung ist das Benutzen im *trainModel.py* Skript eines ImageDataGenerators. Dieser kann die Bilder des DataSets nehmen und zufällige Bilder verändern anhand von angegebenen Parametern. Wichtig zu erwähnen ist, das keine Bilder generiert und den Datenbestand hinzugefügt wird, es werden Bilder im Datenbestand verändert. Somit findet die Datenerweiterung (Data Augmentation) vor Ort statt und wird auch In-Place-Datenerweiterung genannt. (in-place data augmentation).
